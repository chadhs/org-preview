...
Download the Linux x86_64 AppImage or tar archive from [[https://github.com/chadhs/org-preview/releases][GitHub Releases]],
or build on Linux. The application was tested on Omarchy 4.0.2 with
Hyprland 0.56.2, using both native Wayland and XWayland. AppImage
extract-and-run and the extracted tar application both passed.

There is no AUR or pacman package yet. Run the AppImage as your normal
desktop user; the tar archive is an alternative that does not need FUSE.

#+begin_src sh
chmod +x release/*.AppImage
./release/Org\ Preview-VERSION.AppImage ~/notes/today.org
#+end_src
...
requirement. If AppImage reports a FUSE error on Arch, install [[https://archlinux.org/packages/extra/x86_64/fuse2/][fuse2]] or use
[[https://docs.appimage.org/user-guide/troubleshooting/fuse.html][AppImage's extract-and-run option]]:

#+begin_src sh
./release/Org\ Preview-VERSION.AppImage --appimage-extract-and-run --ozone-platform=wayland ~/notes/today.org
#+end_src
...
CI exercises Linux under Xvfb and validates the extracted AppImage launcher.
The AppImage contains desktop metadata and icons; desktop integration tools
can use them to add a launcher. The portable tar archive does not install
a launcher or change file associations.
The Omarchy test machine lacked FUSE 2, so direct FUSE-mounted AppImage
launch was not verified there. The two alternatives above keep sandboxing
enabled.
...
PRs and manual Desktop checks runs only validate builds. Main releases use
...
queue up to 100 pending runs. Do not use =[skip ci]= markers on main commits;
GitHub skips those runs before the release workflow can start.
...
are unsigned and unnotarized; downloaded-app approval remains an installation
check. Keep one installed copy to avoid ambiguous Finder Open With entries.
