...
Org Preview is a live reader for local =.org= files on macOS and Linux.
Keep writing in your editor; previews update when you save.

- *Document tabs* keep several notebooks open, with separate search, scroll
  positions, and live updates for each file.
- *Mermaid diagrams* render directly from Org source blocks, entirely offline,
  with a Show source disclosure and support for the selected theme.

[[#install][Install on macOS or Omarchy]]

* Screenshots
v0.6.0 in Default Light: multiple document tabs, the compact toolbar, and Org
tasks and tables.

[[file:docs/screenshots/preview-light.png]]

v0.6.0 in Solarized Dark: a Mermaid flowchart rendered from an Org source
block, with the diagram source expanded below it.

[[file:docs/screenshots/preview-dark.png]]

* Compact window toolbar
Window controls and reading actions share one compact title bar. The title
bar and document tabs follow the selected theme, including the background
behind the native window buttons. Drag unused toolbar space to move the window.
The tabs keep a full-width row beneath it.

The icon and version badge sit beside the app name. Open *Appearance* to
choose theme preferences. On Linux,
Electron's window-controls overlay reserves space for the native buttons.

* Document tabs
Open several files with the file picker, drag and drop, or command-line paths.
Each document opens in its own tab. Opening the same file again selects its
existing tab. Files with the same name show their parent folders in the tabs.

Tabs remember your search, selected match, preview/source mode, and scroll
position in each view. Every open file stays watched. Background saves update
that document without switching tabs; its preview renders when you select it.
A missing file keeps its last readable content and shows an error on its tab
until it returns.

Use =Ctrl+Tab= and =Ctrl+Shift+Tab= to switch tabs on either platform.
=Cmd+W= on macOS or =Ctrl+W= on Linux closes the active tab. Add Shift to close
the window. Arrow keys, Home, and End navigate a focused tab strip. Closing the
last tab leaves an Open action. Tabs are not restored after quitting.

Up to 20 documents can be open, with a 16 MiB limit per file and 64 MiB of
source files in total. Close a tab to free its watcher and retained content.
Only the active document retains an image cache and mounted preview.

* Mermaid diagrams
Mermaid source blocks render as diagrams automatically when you open or save
an Org file. The renderer is bundled locally; no Emacs, CLI tool, or network
connection is required. Flowcharts and sequence diagrams are covered by the
app's desktop checks. Other diagram types in the bundled Mermaid library can
render, subject to the same limits and offline asset restrictions.
...
Diagrams follow the active appearance theme. Use *Show source* below a diagram
to read or search its text. Source view always shows the original Org document.
Invalid diagrams show their source and an explanation without interrupting the
rest of the preview.

An immediately following =#+RESULTS:= with one local image link is hidden after
successful diagram rendering, so an existing Babel image does not appear twice.
Named blocks require matching result names. If rendering fails, the generated
image remains visible. Unrelated and ambiguous results remain unchanged.

Rendering is limited to 20 diagrams per document, 20,000 characters per block,
100,000 diagram characters per document, 200 edges where the diagram type
supports that limit, 4 MiB of generated SVG per diagram, and five seconds per
render request. Layout runs in a separate sandboxed renderer so a slow diagram
can be stopped without freezing the reader.

Mermaid configuration directives and YAML frontmatter are not supported.
HTML labels, diagram links, callbacks, and external resources are disabled.
Babel headers such as =:file= never write files or run commands. Generate
Graphviz, PlantUML, or ditaa images in your editor and use local image links
for those formats.

* Appearance
Open *Appearance* in the toolbar to choose *System*, *Light*, or *Dark* mode.
Choose a light theme and a dark theme independently:

- *Default Light* and *Default Dark* keep Org Preview's original colors.
- *Solarized Light* and *Solarized Dark* use the [[https://ethanschoonover.com/solarized/][Solarized palette]].

System switches between your preferred themes when your device changes
appearance. Light and Dark stay in the selected mode. Both theme preferences
remain editable in every mode, and changes save immediately for the next launch.
Existing installations keep their mode and use the default theme pair.

Enable *Sans-serif headings* to use the app's sans-serif font for the document
title and headings. This preference applies to every theme and saves immediately.
It starts off, preserving the original heading fonts.

* Syntax highlighting
Preview source blocks use their declared language, such as =#+begin_src python=.
Highlighting works offline and follows the selected color theme.

Bundled languages include Bash, C, C++, CSS, diff, Go, INI/TOML, Java,
JavaScript, JSON, Lisp, Lua, Markdown, Nix, Python, Ruby, Rust, SQL,
TypeScript, XML/HTML, and YAML. Common aliases such as =js=, =ts=, =sh=,
and =yml= work too. =emacs-lisp= and =elisp= use the generic Lisp grammar.

Unknown or missing languages, example/export blocks, and inline code stay
plain text. Blocks over 50,000 characters stay plain text, with a total
highlighting budget of 200,000 characters per document to limit rendering work.
Mermaid blocks use the diagram renderer described above. The read-only Org
source view stays plain text. Babel code never executes.

* Local images
Image links without descriptions display inline, for example
=[[file:images/chart.png]]= or =[[./photo.jpg]]=.
PNG, JPEG, GIF, WebP, and SVG files are supported. Relative paths resolve
from the Org file's folder; absolute paths and =~/= paths also work.
Images fit the reading column without being stretched beyond their size.

Missing, unreadable, or invalid images show a placeholder while the document
remains readable. Images reload when you save the Org file or close and reopen its tab.
Image-only changes are not watched separately. Web images remain links,
and links with descriptions remain text, following Org's inline-image convention.

Previews are limited to 100 image links, 8 MiB per image, and 32 MiB of unique
image data per document. SVGs display in an image context, without executing
scripts or loading external resources. Embedded HTML remains inert.
...
On Wayland, Org Preview avoids Chromium's incompatible Vulkan startup paths
and uses OpenGL acceleration. No extra launcher flags are needed. An explicit
=--ozone-platform=x11= launch keeps the default X11 graphics configuration.
...
Vite bundles renderer dependencies into =dist/=. Packaged apps omit the raw
=node_modules= tree; the main process uses Electron, Node built-ins, and local
modules only. Bundled dependency licenses remain in =THIRD-PARTY-NOTICES.org=.
...
file at a time and supports UTF-8 files up to 16 MiB. Large documents can take
longer to render, depending on their structure.
...
Local image links display inline. Links to other local files are not opened.
...
* Parser patches
Orga 4.7.1 recursively advances through tokens and can exhaust Electron's
JavaScript stack on long sections. =patches/orga+4.7.1.patch= replaces those
tail calls with an iterative loop and appends tokens without copying the
entire token history. =npm ci= applies the patches through =patch-package=
and fails if they no longer apply.
=patches/text-kit+4.5.1.patch= changes character-to-line lookup from a linear
scan to binary search and reuses position helpers between range readers.
Both versions stay pinned until upstream fixes replace these patches.
...
multi-file drops, tabs, Mermaid diagrams, input errors, CLI handoff, saves, atomic replacement,
delete/recreate recovery, background watchers, and supported-link dispatch.
It also opens and live-reloads a document over 5 MiB, checks its source,
search, and outline, and rejects the Wayland Vulkan warning in startup logs.
