...
- The original build warned about the default Electron icon and absent
  =desktopName=. Release preparation subsequently added the project icon,
  matching desktop/window identifiers, and explicit sandbox-preserving launcher
  arguments. CI validates the extracted desktop entry. Installed desktop-file
  association still needs the maintainer's installation check.
